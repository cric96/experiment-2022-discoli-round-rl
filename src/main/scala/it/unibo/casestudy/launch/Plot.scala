...
object Plot extends App {
  ...
  rl.foreach { case (name, data) =>
    ...
    plotRl(data, fixed, adHoc, name)
  }
  store(renderToFile(errorPlot), imageFolder / s"error.png")
  store(renderToFile(totalTickPlot), imageFolder / s"ticks.png")
  // Utility functions
