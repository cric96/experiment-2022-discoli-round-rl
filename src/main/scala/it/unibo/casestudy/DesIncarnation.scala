...
object DesIncarnation extends BasicAbstractSimulationIncarnation
  with SpatialSimulation
  with BasicSpatialAbstraction
  with DiscreteEventSimulation
  with StandardLibrary {
  override type P = Point3D
  override implicit val idBounded: DesIncarnation.Builtins.Bounded[Int] = DesIncarnation.Builtins.Bounded.of_i
}
